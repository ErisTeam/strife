...
export function TabList(props: TabsProps) {
  ...
  function onDragEnd(event: DragEvent) {
    if (event.droppable) {
      ...
      for (let i = 0; i < items().length; i++) {
        ...
        if (items()[i].id === (event.droppable.id as number) - 1)
          dropTabIndex = i;
        if (items()[i].id === (event.draggable.id as number) - 1)
          dragTabIndex = i;
      }
      setItems((items) => {
        ...
        <nav class={[props.className, style.tabs].join(' ')}>
          <ul>
            <SortableProvider ids={ids()}>
              <For each={items()}>
                {(item) => <TabSortable tab={item.tab} id={item.id} />}
              </For>
            </SortableProvider>
          </ul>
        </nav>
