...
export function SettingsEntryElement(props: {
  setting: SettingsEntry;
  index: number;
}) {
  const value = createMemo(() => {
    let value =
      props.setting.value == null
  ...
  function updateSetting(e: any) {
    let value: Date | string | number | boolean | CssPropertyType;
  ...
  return (
    <section
      class={style.entry}
      classList={{
        [style.showResetButton]:
          props.setting.value != null &&
          props.setting.value !== props.setting.defaultValue,
      }}
    >
      ...
      <span class={style.input}>
        ...
        <button
          type="button"
          class={`${style.showResetButton} ${style.resetButton}`}
          onclick={reset}
