...
type ContextMenus<D> = ContextMenuData<D> & {
  ref?: HTMLOListElement;
  index: number;
};

export const ContextMenusProvider = <D,>(props: {
  ...
  function clickOutside(e: MouseEvent) {
    setContextMenus(
      (contextMenu) =>
        contextMenu.visible && !contextMenu.ref?.contains(e.target as Node),
  ...
  const value = {
    ...
    removeContextMenu: (index: number) => {
      if (index === contextMenus.length - 1) {
        setContextMenus(
          produce((contextMenus) => contextMenus.splice(index, 1))
        );
      } else {
...
export function createContextMenu<D>(contextMenu: ContextMenuData<D>) {
  ...
  return Object.defineProperties(onContextMenu, a) as typeof onContextMenu &
    typeof functions;
}
...
function ContextMenu<T>(props: ContextMenuProps<T>) {
  ...
  const pos = { x: props.position.x, y: props.position.y };
  ...
  return (
    <ol
      ref={ref}
      style={{ left: `${pos.x}px`, top: `${props.position.y}px` }}
      class={style.contexMenu}
    >
      <MenuProvider
        closeMenu={() => c.setvisiblity(props.index, false)}
        data={props.data}
      >
        {props.children}
      </MenuProvider>
