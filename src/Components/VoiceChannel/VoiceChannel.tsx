...
function Log(props: { date: Date; message: string }) {
  ...
  return (
    <div use:contextMenu>
      <span class={style.time}>[{formatTime(props.date)}]</span>&nbsp
      {props.message}
    </div>
  );
}
...
export function VoiceChannel() {
  ...
  async function startVoiceConnection() {
    ...
    const voiceStateUpdate = await gatewayOneTimeListener(
      AppState.userId(),
      'voiceStateUpdate'
    );
    ...
    setLogs(logs.length, {
      date: new Date(),
      message: JSON.stringify(voiceStateUpdate),
    });
    const voiceServerUpdate = await gatewayOneTimeListener(
      AppState.userId(),
      'voiceServerUpdate'
    );
    ...
    setLogs(logs.length, {
      date: new Date(),
      message: JSON.stringify(voiceServerUpdate),
    });
    ...
    <div>
      ...
      <article class={style.controls}>
        ...
        <div class={style.sendContainer}>
          <textarea cols="30" rows="10" placeholder="message to send" />
          <button type="button" class={buttons.default}>
