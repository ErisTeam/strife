	...
	batch(() => {
		...
		if (!keepOrder) {
			AppState.setTabsOrder((prev) => {
				...
				const returnable = prev.map((order) => {
					if (order > tabIndex) {
						return order - 1;
					}
					return order;
...
export function add(tab: Tab, replaceCurrent = false) {
	...
	batch(() => {
		...
		AppState.setTabsOrder((prev) => {
			const returnable = prev.map((index) => {
				if (index > tabIndex) {
					return index - 1;
				}
				return index;
			});
			returnable[currentOrder] = newIndex;
			return returnable;
		});
		console.log("currentOrder", currentOrder, AppState.tabsOrder());
	});
...
export async function saveToFile() {
	...
	const newTabs = unwrap(AppState.tabs)
		.slice()
		.map((tab) => {
...
export async function loadFromFile(): Promise<boolean> {
	...
	const doesFileExist = await exists(tabsPath, { baseDir: BaseDirectory.AppData });
	if (!doesFileExist) {
		console.warn("No session data tabs file found");
		return false;
	}

	const tabsFile = JSON.parse(await readTextFile(tabsPath, { baseDir: BaseDirectory.AppData })) as TabsFile;
