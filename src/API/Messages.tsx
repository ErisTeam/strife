...
const codeRulesPreserve = [
  ...
  [
    regex.insides.code,
    '<span class="mdHint">&grave;</span><code>$2</code><span class="mdHint">&grave;</span>',
  ],
];
const rulesPreserve = [
  [
  ...
  [
    regex.insides.italic,
    '<span class="mdHint">&ast;</span><i>$2</i><span class="mdHint">&ast;</span>',
  ],
  [
  ...
  [
    regex.insides.alternateItalic,
    '<span class="mdHint">&lowbar;</span><i>$2</i><span class="mdHint">&lowbar;</span>',
  ],
  [
  ...
  [
    regex.insides.quote,
    '<span class="mdHint">&gt; </span><blockquote>$1</blockquote>',
  ],
];
...
export function formatMarkdownToHTMLPreserve(c: string, sanitize = true) {
  ...
  for (const c of combined) {
    ...
    for (const rule of rulesPreserve) {
      ...
      if (match) {
        const replaced = c.replaceAll(regex, replacement as string);
        combined[combined.indexOf(c)] = formatMarkdownToHTMLPreserve(
          replaced,
          false
        );
      }
...
export function formatMentions(
  content: string,
  mentionsInput: [{ id: string; username: string }]
  ...
  const mentions =
    content.match(mentionsRegex)?.map((match) => {
      ...
      if (match.match(userMentionRegex)) {
        element = (
          <UserMention
            mentioned_user={mentionsInput.find((mention) =>
              match.includes(mention.id)
            )}
          />
        );
...
export async function sendMessage(
  ...
  const content = contentInput.replaceAll(mentionReplaceRule, (match) => {
    const user = usernameIdPairs.find(
      (pair) => pair.username === match.substring(1)
    );
