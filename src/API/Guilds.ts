...
import type { Snowflake } from "@/types/utils";
import murmurhash from "murmurhash";
import { produce } from "solid-js/store";
export function getGuildIconFromChannel(channel: Channel): string {
  const AppState = useAppState();
  const guild = AppState.userGuilds.find(
    (g) => g.properties.id === channel.guild_id
  );
...
export async function updateGuilds() {
  ...
  for (const guild of guilds) {
    const categories: ChannelWithChildren[] = guild.channels.filter(
      (channel) => !channel.parent_id
    );
    for (const category of categories) {
      category.children = guild.channels.filter(
        (channel) => channel.parent_id === category.id
      );

      category.children.sort(
        (a: Channel, b: Channel) => b.type - a.type || b.position - a.position
      );
    }

    categories.sort(
      (a: ChannelWithChildren, b: ChannelWithChildren) =>
        a.type - b.type || a.position - b.position
    );
    guild.channels = [];
...
export function addAdditionalGuildDataToState(data: GuildListUpdate) {
  const AppState = useAppState();
  ...
  console.log(
    "Added additional data to state",
    AppState.openedGuildsAdditionalData
  );
}

function murumurhash3(key: string, seed = 0) {
  let h1 = seed;
  let h1b: number;
  const c1 = 0xcc9e2d51;
  let c1b: number;
...
export function getListIdForChannel(channel: Channel) {
  ...
  const hashOut = murumurhash3(HashIn.join(","), 0);
