...
import type { CssPropertyType } from '../Components/Settings/SettingsTypes';
...
export function start() {
  const CSSSettingsVariables = {
    ...
    [StyleIds.Guilds.fallbackBackground]:
      CssVariables.Components.Guilds.fallbackBackground,
    [StyleIds.Guilds.dividerColor]: CssVariables.Components.Guilds.dividerColor,
    [StyleIds.Guilds.tooltipBackground]:
      CssVariables.Components.Guilds.tooltipBackground,
    [StyleIds.Guilds.beforeBackground]:
      CssVariables.Components.Guilds.beforeBackground,
    [StyleIds.Guilds.width]: CssVariables.Components.Guilds.width,

    [StyleIds.Channels.categoryText]:
      CssVariables.Components.Channels.categoryText,
    [StyleIds.Channels.iconColor]: CssVariables.Components.Channels.iconColor,
    [StyleIds.Channels.iconBackground]:
      CssVariables.Components.Channels.iconBackground,
    [StyleIds.Channels.categoryRoundness]:
      CssVariables.Components.Channels.categoryRoundness,

    [StyleIds.Misc.loadingBackground]:
      CssVariables.Components.Misc.loadingBackground,
  };
  ...
  function updateSettingsEntry(event: {} & { detail: { id: string } }) {
    ...
    const property = document.body.style.getPropertyValue(variable);
    if (
      entry.value == null ||
      (entry.value === entry.defaultValue && property == null)
    ) {
      return;
    }
    ...
    switch (entry.type) {
      case 'CssNumberProperty':
        stringValue = entry.value.value + entry.value.unit;
        break;
    }
    ...
    document.documentElement.style.setProperty(variable, stringValue);
  }
  //TODO:Fix types
...
export function CssStringVariableToObject(variable: string): CssPropertyType {
  const [value, unit] = variable.split(/([a-z]+)/);
  return {
...
export function hslToHex(h: number, s: number, lInput: number) {
  const l = lInput / 100;
