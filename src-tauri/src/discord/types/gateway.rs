...
pub mod gateway_packets_data {
    ...
    use crate::discord::types::{
        ...
        user::{CurrentUser, GuildSettings, PublicUser},
        SnowFlake,
    };

    use self::guild_member_list_update::{GuildGroup, Ops};

    use super::{ClientState, Presence, Properties, ReadState};
    ...
    fn serialize_lazy_guilds_channels<S>(
        value: &Option<LazyGuildsChannels>,
        serializer: S,
    ) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
