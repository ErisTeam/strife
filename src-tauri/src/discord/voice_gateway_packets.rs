...
use super::types::gateway::voice_gateway_packets_data::{
    Heartbeat, HeartbeatAck, Hello, Identify, Ready, SelectProtocol, UserInfo, UserInfo2,
};
...
impl<'de> Deserialize<'de> for IncomingPacket {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let packet_data = match inner.op {
            OpCode::Ready => IncomingPacketsData::Ready(
            ...
            OpCode::UserInfo => IncomingPacketsData::UserInfo(
                serde_json::from_value(inner.d).map_err(serde::de::Error::custom)?,
            ),
            OpCode::UserInfoPlatform => IncomingPacketsData::UserInfoPlatform(
                serde_json::from_value(inner.d).map_err(serde::de::Error::custom)?,
            ),
        };
        Ok(Self {
