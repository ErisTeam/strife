...
use crate::{
    ...
    modules::{auth::Auth, main_app::MainApp, user_manager},
};
...
pub async fn set_state(
    ...
    match new_state.as_str() {
        "Application" => {
            ...
            state
                .change_state(new_state, handle)
                .await
                .map_err(|e| GeneralError::Other(e.to_string()))?;
        }
        "LoginScreen" => {
            let mut auth = Auth::new(Arc::downgrade(&state));
            auth.start_gateway(handle.clone())
                .await
                .map_err(|e| GeneralError::Other(e.to_string()))?;
...
pub async fn close_splashscreen(sender_window: tauri::Window) -> core::result::Result<(), String> {
    // Close splashscreen
    if let Some(splashscreen) = sender_window.get_webview_window("splashscreen") {
        splashscreen.close().unwrap();
    }

    // Show main window
    let window = sender_window
        .get_webview_window("main")
        .ok_or("main window is None")?;
...
pub async fn get_token(
    ...
    Ok(state
        ...
        .token
        .ok_or(GeneralError::TokenNotFound)?
        .to_string())
}
