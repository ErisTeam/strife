...
mod tests {
    mod gateway {
        ...
        use crate::discord::{
            gateway_packets::{DispatchedEvents, IncomingPacket, IncomingPacketsData},
            types::gateway::gateway_packets_data::TypingStart,
        };
        #[test]
        fn hello() {
            let json: IncomingPacket =
        ...
        fn ready() {
            ...
            match json.data {
                IncomingPacketsData::DispatchedEvent(data) => {
                    assert!(matches!(data, DispatchedEvents::Ready(_)))
                }
                d => {
        ...
        fn ready_supplemental() {
            let json: IncomingPacket = serde_json::from_reader(
        ...
        fn typing_start() {
            let json: IncomingPacket =
                serde_json::from_reader(fs::File::open("../tests/typing start.json").unwrap())
                    .unwrap();
