...
use crate::discord::types::gateway::gateway_packets_data::{
    GuildMemberListUpdate, GuildMembersChunk, LazyGuilds, MessageEvent, TypingStart,
    VoiceServerUpdate, VoiceStateUpdate,
};
use crate::discord::types::guild::strife::{Group, GroupType, GuildListData};
...
impl MainApp {
    ...
    pub async fn activate_user(
        ...
        if users
            .insert(user_id, ActivationState::Activated(user))
            .is_some()
        {
            ...
            match message {
                ...
                GatewayMessages::Ready(data) => {
                    let user_data = user_data
                        .upgrade()
                        .ok_or("User data is no longer available")?;
                    ...
                    for item in &data.ops {
                        match &item.payload {
							crate::discord::types::gateway::gateway_packets_data::guild_member_list_update::OpData::Sync(
								...
								handle.emit("gateway", webview_packets::GatewayEvent {
    ...
    async fn start_gateway(
        ...
        Arc<RwLock<Gateway>>,
        tokio::sync::mpsc::Receiver<GatewayMessages>,
    )> {
