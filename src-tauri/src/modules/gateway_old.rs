...
impl Gateway {
					...
					if let Some(user_data) = &user_data {
						...
						if let Some(guild) = guild {
							...
							if let Some(settings) = settings {
								let channel = settings.channel_overrides
									.iter()
	...
	async fn init_message(
		...
		client.send(Message::Text(serde_json::to_string(&data).unwrap())).await.unwrap();
	}
	...
	fn on_close(&self, frame: Option<CloseFrame>) -> Result<GatewayResult, GatewayError> {
		if let Some(frame) = frame {
			match frame.code {
				...
				tokio_tungstenite::tungstenite::protocol::frame::coding::CloseCode::Library(4010 | 4011) => {
					error!("how? also gami is a furry");
					return Ok(GatewayResult::Reconnect);
				}
				tokio_tungstenite::tungstenite::protocol::frame::coding::CloseCode::Library(4012) => {
					error!("invalid api version");
					return Err(GatewayError::InvalidApiVersion);
				}
				tokio_tungstenite::tungstenite::protocol::frame::coding::CloseCode::Library(4013 | 4014) => {
		...
		let recive_loop = async move {
			loop {
				...
				if let Some(rec) = rec {
					...
					return ReciveLoopError::ReciveError(tokio_tungstenite::tungstenite::Error::ConnectionClosed);
				}
			}
		};
						...
						tokio_tungstenite::tungstenite::Message::Binary(bin) => {
							...
							if last_4 == [0, 0, 255, 255] {
								...
								if cfg!(debug_assertions) {
									...
									let packet = serde_json::from_str::<GatewayIncomingPacket>(out.as_str());
									let r#type;
