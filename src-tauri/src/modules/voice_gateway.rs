...
use crate::{
    discord::{
        ...
        voice_gateway_packets::{IncomingPacket, OutGoingPacket, OutGoingPacketsData},
    },
    webview_packets,
};

use super::{gateway_utils::Errors, main_app::VoiceGatewayMessages};
...
impl VoiceGateway {
    ...
    pub async fn start(
        ...
        let (message_sender, message_reciver) =
            tokio::sync::mpsc::channel::<VoiceGatewayMessages>(5);
        ...
        let connection_info = Arc::new(Mutex::new(self.create_connection_info(
            sender,
            handle,
            voice_token,
            guild_id,
    ...
    async fn heartbeat_thread(
        ...
        loop {
            ...
            let packet = serde_json::to_string(&OutGoingPacket::new(
