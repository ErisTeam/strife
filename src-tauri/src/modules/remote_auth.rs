...
use crate::discord::{
    constants,
    remote_auth_packets::{IncomingPackets, OutGoingPackets},
};

use super::{auth::RemoteAuthMessages, gateway_utils::ConnectionInfo};
...
impl MobileAuth {
    ...
    async fn logic_thread(
        ...
        >,
        connection_info: Arc<
            tokio::sync::Mutex<ConnectionInfo<MobileAuthConnectionData, RemoteAuthMessages>>,
        >,
                ...
                tokio_tungstenite::tungstenite::Message::Text(data) => {
                    ...
                    match event {
                        ...
                        IncomingPackets::NonceProof { encrypted_nonce } => {
                            ...
                            let packet =
                                serde_json::to_string(&(OutGoingPackets::NonceProof { proof }))?;
                        ...
                        IncomingPackets::Cancel {} => {
                            ...
                            connection_info
                                .sender
                                .send(RemoteAuthMessages::CancelQrCode)
                                .await?;
                        ...
                        IncomingPackets::PendingRemoteInit { fingerprint } => {
                            ...
                            connection_info
                                .sender
                                .send(RemoteAuthMessages::UpdateQrCode { fingerprint })
                                .await?;
                        }
                        IncomingPackets::PendingTicket {
                            encrypted_user_payload,
                        } => {
                            let connection_info = connection_info.lock().await;

                            let decrypted =
                                connection_info.aditional_data.tak(encrypted_user_payload)?;
                            ...
                            connection_info
                                .sender
                                .send(RemoteAuthMessages::UpdateQrUserData {
                                    user_id,
                                    discriminator,
                                    avatar_hash,
                                    username,
    ...
    ) -> std::result::Result<tokio::sync::broadcast::Receiver<String>, Box<dyn std::error::Error>>
    {
        ...
        let connection_info = Arc::new(Mutex::new(
            self.create_connection_info(auth_sender, handle)?,
        ));
