...
use crate::{
    discord::{
        constants,
        gateway_packets::{DispatchedEvents, IncomingPacket, OutGoingPacket, OutGoingPacketsData},
        types::gateway::gateway_packets_data::{
            Identify, LazyGuilds, RequestGuildMembers, Resume, VoiceStateUpdateSend,
        },
        user::UserData,
    },
...
use super::{gateway_utils::Errors, main_app::GatewayMessages};
...
impl Gateway {
    ...
    fn create_connection_info(
        ...
        Ok(ConnectionInfo::new(
            ConnectionData::new(token, is_recconecting),
            app_handle,
            sender,
        ))
    }
            ...
            match message {
                tokio_tungstenite::tungstenite::Message::Binary(payload) => {
                    ...
                    let packet = if cfg!(debug_assertions) {
                        ...
                        let p = connection_info.handle.path().app_data_dir().unwrap();
                        //    println!("{:?}", p);
                    ...
                    match packet.data {
                        crate::discord::gateway_packets::IncomingPacketsData::Hello(data) => {
                            ...
                            connection_info.heartbeat_interval =
                                Duration::from_millis(data.heartbeat_interval);
                        ...
                        crate::discord::gateway_packets::IncomingPacketsData::Heartbeat(_) => {
                            ...
                            websocket
                                .send(tokio_tungstenite::tungstenite::Message::Text(
                                    serde_json::to_string(&OutGoingPacket::heartbeat(
                                        connection_info.aditional_data.sequence_number,
                                    ))?,
                                ))
                                .await?;
                        }
                        ...
                        crate::discord::gateway_packets::IncomingPacketsData::DispatchedEvent(
                            data,
                        ) => {
                            let mut connection_info = connection_info.lock().await;
                            match data {
                        ...
                        crate::discord::gateway_packets::IncomingPacketsData::Reconnect => {
                            ...
                            websocket
                                .reconnect(connection_info.aditional_data.resume_url.clone())
                                .await?;
                            ...
                            let last_sequece_number = connection_info
                                .aditional_data
                                .sequence_number
                                .as_ref()
                                .unwrap()
                            ...
                            let packet =
                                OutGoingPacket::new(OutGoingPacketsData::Resume(Resume {
                                    token: connection_info.aditional_data.token.clone(),
                                    session_id,
    ...
    pub async fn start(
        ...
        let connection_info = Arc::new(Mutex::new(
            self.create_connection_info(sender, handle, token, false)?,
        ));
