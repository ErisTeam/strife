...
impl Auth {
        ...
        match res {
            LoginResponse::Success {
                ...
                state
                    ...
                    .add_user(
                        user_id.clone(),
                        crate::modules::user_manager::User {
                ...
                response = webview_packets::auth::Auth::LoginSuccess {
                    user_id,
            ...
            LoginResponse::Error {
                ...
                response = webview_packets::auth::Auth::Error {
                    code,
                    errors,
                    message,
                };
        ...
        loop {
            ...
            if let Some(payload) = payload {
                match payload {
                    RemoteAuthMessages::Login {
                        ...
                        match res {
                            http_packets::auth::LoginResponse::Success { encrypted_token } => {
                                ...
                                let decrypted =
                                    private_key.decrypt(Oaep::new::<sha2::Sha256>(), &decoded)?;
                                ...
                                handle.emit(
                                    "auth",
                                    webview_packets::auth::Auth::LoginSuccess {
                                        user_id,
                                        user_settings: None,
                                    },
                                )?;
                            ...
                            http_packets::auth::LoginResponse::RequireAuth {
                                ...
                                let captcha_sitekey =
                                    captcha_sitekey.ok_or("captcha_sitekey is none")?;
                                let captcha_data =
                                    captcha_data.upgrade().ok_or("captcha_data is none")?;
                                captcha_data.write().await.replace(CaptchaData {
                                    captcha_data: captcha_rqdata.ok_or("captcha_rqdata is none")?,
                                    captcha_token: captcha_rqtoken
                                        .ok_or("captcha_rqtoken is none")?,
                                    captcha_sitekey: captcha_sitekey.clone(),
                                });
                                handle.emit(
                                    "auth",
                                    webview_packets::auth::Auth::RequireAuth {
                                        captcha_key,
                            ...
                            http_packets::auth::LoginResponse::Error {
                                code,
                                errors,
                                message,
                            } => {
                                ...
                                handle.emit(
                                    "auth",
                                    webview_packets::auth::Auth::Error {
                                        code,
                                        errors,
                                        message,
                                    },
                    ...
                    RemoteAuthMessages::UpdateQrUserData {
                        user_id,
                        discriminator,
                        avatar_hash: avatar,
                        username,
                        ...
                        handle.emit(
                            "auth",
                            webview_packets::auth::Auth::MobileTicketData {
                                user_id,
                                discriminator,
                                avatar_hash: avatar,
                                username,
                    ...
                    RemoteAuthMessages::UpdateQrCode { fingerprint } => {
                        handle.emit(
                            "auth",
                            webview_packets::auth::Auth::MobileQrcode {
                    ...
                    RemoteAuthMessages::CancelQrCode => {
                        handle.emit(
                            "auth",
                            webview_packets::auth::Auth::MobileQrcode { qrcode: None },
                        )?;
    ...
    pub async fn login_remote_auth(
        ...
        http_packets::auth::LoginResponse,
    ...
    > {
        ...
        let mut request = client
            ...
            .header(
                "x-super-properties",
                Properties::default().base64().unwrap(),
            )
            .body(serde_json::to_string(
                &(http_packets::auth::mobile_auth::Login { ticket }),
            )?);
        ...
        if cfg!(debug_assertions) {
            ...
            println!("{:?}", res);
            return Ok(serde_json::from_str::<http_packets::auth::LoginResponse>(
                &res,
            )?);
        }
