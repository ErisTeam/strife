...
impl EventManager {
    ...
    pub fn register_debug(&self, handle: AppHandle) {
        ...
        event_listeners.push(handle.listen_global("testReconnecting", move |event| {
    ...
    pub fn register_for_login_screen(&self, handle: AppHandle) {
        ...
        event_listeners.extend(events::auth::get_all_events(
            self.state.clone(),
            handle.clone(),
        ));
    }
    pub fn register_for_main_app(&self, handle: AppHandle) {
        let mut event_listeners = self.event_listeners.lock().unwrap();
        event_listeners.extend(events::main_app::get_all_events(
            self.state.clone(),
            handle.clone(),
        ));
    }
