...
use tauri::{async_runtime::TokioHandle, Emitter, Event, EventId, Listener, Manager};
...
fn get_user_data(state: Arc<MainState>, handle: tauri::AppHandle) -> impl Fn(Event) {
    move |event| {
        let user_id = serde_json::from_str::<GetUserData>(event.payload())
            .unwrap()
            .user_id;
        ...
        let payload = block_in_place(move || {
            TokioHandle::current().block_on(async move {
                ...
                if let Some(crate::modules::main_app::ActivationState::Activated(data)) =
                    users.get(&user_id)
                {
        ...
        handle.emit("general", payload).unwrap();
    }
}
            ...
            TokioHandle::current().block_on(async move {
                    ...
                    users.get(&user_id)
                {
                    if let Some(user_data) = data.read_user_data().await.as_ref() {
                        ...
                        res = General::Relationships { relationships };
                    }
                }
                res
            })
...
trait EventTrait<T: serde::Serialize + Sized + Clone + Debug>: serde::de::DeserializeOwned {
    ...
    fn register(state: &Arc<MainState>, handle: &tauri::AppHandle) -> EventId {
        info!("Registering event {}", Self::get_name());
        let h = handle.clone();
        let state = state.clone();
        handle.listen(Self::get_name(), move |event: Event| {
            info!(
                "got event with payload {} {:?}",
                Self::get_name(),
                event.payload()
            );
            let json: Self = serde_json::from_str(event.payload()).unwrap();
            let res = json.execute(state.clone(), h.clone());
            if let Some((name, data)) = res {
                h.emit(name, data).unwrap();
            }
        })
    }
}
...
pub fn get_all_events(
    ...
    let h = handle.clone();
    vec![
        handle.listen("getUserData", get_user_data(state.clone(), h.clone())),
        handle.listen(
            "getRelationships",
