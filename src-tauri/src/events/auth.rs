...
use tauri::{async_runtime::TokioHandle, Emitter, Event, EventId, Listener};
...
fn verify_login(state: Arc<MainState>, handle: tauri::AppHandle) -> impl Fn(Event) {
    move |event| {
        let payload = serde_json::from_str::<VerifyLoginPayload>(event.payload());
        if payload.is_err() {
            handle
                .emit(
                    "auth",
                    webview_packets::auth::MFA::VerifyError {
        ...
        handle.emit("auth", MFA::from(payload.unwrap())).unwrap();
    }
}
...
fn login(state: Arc<MainState>, handle: tauri::AppHandle) -> impl Fn(Event) {
    move |event| {
        let payload: LoginPayload = serde_json::from_str(event.payload()).unwrap(); //TODO: handle error
        ...
        let res = block_in_place(move || {
            TokioHandle::current().block_on(async move {
                ...
                auth.login(payload.captcha_token, payload.login, payload.password)
                    .await
            })
        });
        ...
        handle.emit("auth", payload).unwrap();
    }
}
...
fn login_remote_auth(state: Arc<MainState>, handle: tauri::AppHandle) -> impl Fn(Event) {
    move |event| {
        let payload: MobileAuthLoginPayload = serde_json::from_str(event.payload()).unwrap();
...
pub fn get_all_events(
    ...
    let h = handle.clone();
    vec![
        h.listen("sendSms", send_sms(state.state.clone(), handle.clone())),
        h.listen("verifyLogin", verify_login(state.clone(), handle.clone())),
        h.listen("login", login(state.clone(), handle.clone())),
        h.listen(
            "loginMobileAuth",
            login_remote_auth(state.clone(), handle.clone()),
        ),
    ]
}
